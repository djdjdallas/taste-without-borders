...
const SignUpScreen = () => {
  ...
  const handleUserSignUp = async () => {
    ...
    try {
      ...
      if (!signUpResponse.userConfirmed) {
        try {
          await AsyncStorage.setItem("unconfirmedEmail", email);
          console.log("Stored email for verification:", email);
          setIsVerifying(true);
        } catch (storageError) {
          console.error("Failed to save email for verification:", storageError);
          Alert.alert(
            "Sign Up Error",
  ...
  const handleVerification = async () => {
    let savedEmail;
    try {
      savedEmail = await AsyncStorage.getItem("unconfirmedEmail");
      console.log("Retrieved email for verification:", savedEmail);
      if (!savedEmail) throw new Error("Email not found in storage.");
    } catch (storageError) {
      console.error("Failed to retrieve email for verification:", storageError);
      Alert.alert(
        "Verification Error",
    ...
    try {
      ...
      setIsVerifying(false);
      await AsyncStorage.removeItem("unconfirmedEmail");
      navigation.navigate("Allergy");
    } catch (error) {
