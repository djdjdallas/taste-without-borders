...
const HomeScreen = () => {
  ...
  const fetchUserInfo = async () => {
    try {
      ...
      const usernamePart = userEmail.split("@")[0]; // Truncating the email at '@'

      setUserInfo({ ...user, email: userEmail, usernamePart });
      const storedUsername = await AsyncStorage.getItem("username");
      setUsername(storedUsername || user.username);

      console.log("Fetched user info:", user);
      console.log("Truncated email (username part):", usernamePart); // Logging the truncated email
